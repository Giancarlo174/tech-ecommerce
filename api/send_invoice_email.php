<?php
// Control estricto de salida para asegurar que el JSON sea la única salida
while (ob_get_level()) {
    ob_end_clean();
}
...
require_once __DIR__ . '/../includes/functions.php'; // Para formatPrice




//Lógica principal para generar y enviar la factura ---

try {
    // --- A. Validación y Obtención de Datos ---
    
    $orderId = $_POST['order_id'] ?? $_GET['order_id'] ?? null;

    if (!isset($orderId) || !is_numeric($orderId)) {
        throw new Exception('ID de pedido no válido o no proporcionado.');
    }
    ...
    // 1. Obtener información del pedido (incluyendo el correo del cliente)
    ...
    $customerEmail = $order['correo_usuario'] ?? null;
    if (empty($customerEmail)) {
        // En un caso real, aquí se debe usar el correo de guest checkout si aplica
        throw new Exception('El pedido no tiene un correo de usuario asociado para el envío.');
    }
    ...
    $pdf->Cell(95, 6, 'Correo: ' . $customerEmail, 1, 1, 'L');
    ...
    // **CAMBIO CLAVE: Obtener la SALIDA como STRING ('S')**
    $pdfContent = $pdf->Output('', 'S');
    $filename = 'Factura_Pedido_' . $order['id'] . '_' . date('Y-m-d') . '.pdf';


    // --- C. Envío por Correo con ReSend (uso directo de la API vía cURL, sin SDK) ---
    $apiKey = defined('RESEND_API_KEY') ? RESEND_API_KEY : ($_ENV['RESEND_API_KEY'] ?? getenv('RESEND_API_KEY') ?? '');
    if (empty($apiKey)) {
        throw new Exception('Falta RESEND_API_KEY en la configuración (.env o config.php).');
    }

    $fromAddr = trim($_ENV['RESEND_FROM'] ?? getenv('RESEND_FROM') ?? '');
    if (!filter_var($fromAddr, FILTER_VALIDATE_EMAIL)) {
        throw new Exception('Define RESEND_FROM en .env con un email válido y dominio verificado en Resend.');
    }
    $fromEmail = SITE_NAME . ' <' . $fromAddr . '>';

    $resendUrl = 'https://api.resend.com/emails';
    
    // Usar correo del cliente; Esto solo funciona cuando se tiene un dominio verificado en Resend, por ahora se usa el correo de registro
    //$toEmail = $customerEmail; // correo del usuario que compró
    // O si quieres desde .env:
    $toEmail = $_ENV['RESEND_TO_EMAIL'] ?? $customerEmail;
    
    $payload = [
        'from' => $fromEmail,
        'to' => $toEmail,  // string o array ['email@example.com']
        'subject' => SITE_NAME . ' - Tu Factura de #' . $order['id'],
        'html' => '<p>Estimado cliente,<br><br>Adjunto a este correo encontrarás la factura de tu pedido <strong>#' . $order['id'] . '</strong>.<br><br>Saludos,<br>' . SITE_NAME . '</p>',
        'attachments' => [
            [
                'filename' => $filename,
    ...
    if ($resp === false || $httpCode >= 400) {
        throw new Exception('Error enviando con Resend: ' . $curlErr . ' - HTTP ' . $httpCode . ' - ' . $resp);
    }

    echo json_encode(['success' => true, 'message' => 'Factura enviada con éxito.', 'resend_http_code' => $httpCode, 'resend_response' => json_decode($resp, true)]);
    exit;
